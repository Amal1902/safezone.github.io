...
// Function to retrieve and display the first 20 top stories with their links to Hacker News comments
const showTopStories = async () => {
    try {
        ...
        // Fetch details for each top story, limited to the first 20
        for (const storyId of storyIds) {
            if (count >= 20) break; // Stop the loop if we have reached 20 top stories
            ...
            if (storyData.url) {
                ...
                document.getElementById('top-stories').appendChild(listItem);
...
// Call the function to display the first 20 top stories with their links to comments
showTopStories();
